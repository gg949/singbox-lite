...
_add_hysteria2() {
    ...
    if [ "$BATCH_MODE" = "true" ]; then
        ...
        read -p "请输入限速 Mbps (回车默认不限速): " speed_limit
    ...
    local inbound_json=$(jq -n --arg t "$tag" --arg p "$port" --arg pw "$password" --arg op "$obfs_password" --arg spd "$speed_limit" --arg cert "$cert_path" --arg key "$key_path" \
        '{"type":"hysteria2","tag":$t,"listen":"::","listen_port":($p|tonumber),"users":[{"password":$pw}],"down_mbps":($spd | tonumber?),"up_mbps":($spd | tonumber?),
"tls":{"enabled":true,"alpn":["h3"],"certificate_path":$cert,"key_path":$key}} | if $op != "" then .obfs={"type":"salamander","password":$op} else . end | if $spd == "" or $spd == "0" then del(.down_mbps, .up_mbps) else . end')
    _atomic_modify_json "$CONFIG_FILE" ".inbounds += [$inbound_json] | .inbounds |= unique_by(.tag)" || return 1
